...
def mongodb_toolset():
    mongodbToolset = McpToolset(
            connection_params=StreamableHTTPConnectionParams(
                url="https://mcp-server-441019053171.us-west1.run.app/mcp",
            )
        )
...
root_agent = LlmAgent(
  name='NASA_Space_Tracker_Agent',
  model='gemini-2.5-pro',
  description=(
      'NASA Space Tracker Agent — retrieves near-Earth object (NEO) hazard data and satellite tracking data (position, altitude, velocity) from MongoDB.'
  ),
  sub_agents=[],
